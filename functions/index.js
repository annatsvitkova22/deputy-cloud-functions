...
exports.createAppeal = functions.firestore.document('appeals/{appealId}').onCreate( async (snap, context) => {
        ...
        let userEmail;
        try {
            await userRef.get().then(span => {
                userEmail = span.data().email;
                const text = 'Доброго дня Для Вас було створено новий запит, перегляньте його deputy-app';
                const msg = {
                    to: userEmail,
                    from: 'annatsvitkova73@gmail.com',
                    subject: 'Створено новий запит',
                    text: text,
                    html: `<h1>Доброго дня</h1><p>Для Вас було створено новий запит, перегляньте його <a href='https://deputy-app'>deputy-app</a></p>`,
                };
                sgMail.send(msg)
                .then(() => {
                    return true;
